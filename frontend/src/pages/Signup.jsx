...
function useT() {
  ...
  return {
    ...
    success:    e ? "text-green-400" : "text-[#a855f7]",
...
function Particles({ t }) {
  const items = [
    { top: "8%",  left: "10%", size: 6,  delay: 0,   dur: 4   },
    { top: "18%", left: "87%", size: 9,  delay: 0.5, dur: 5   },
    { top: "50%", left: "4%",  size: 5,  delay: 1,   dur: 3.5 },
    { top: "78%", left: "90%", size: 6,  delay: 1.5, dur: 4.5 },
    { top: "38%", left: "94%", size: 4,  delay: 0.8, dur: 3   },
    { top: "88%", left: "12%", size: 7,  delay: 0.3, dur: 5   },
  ];
  const colors = t.e
...
function PasswordStrength({ password, t }) {
  ...
  const colors = ["", "bg-red-500", "bg-yellow-400", "bg-blue-400", t.e ? "bg-green-400" : "bg-[#e040a0]"];
  const textColors = ["", "text-red-400", "text-yellow-400", "text-blue-400", t.e ? "text-green-400" : "text-[#e040a0]"];
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 transition-colors duration-300 ${t.bg}`}>
      ...
      <div className="relative w-full max-w-sm mx-auto z-10">
        ...
        {/* Card */}
        <div
          className={`p-6 sm:p-8 rounded-3xl border-2 shadow-2xl backdrop-blur-md fade-up-1 ${t.card}`}
          style={{ boxShadow: t.e ? "0 24px 64px rgba(34,197,94,0.12)" : "0 24px 64px rgba(14,10,30,0.5)" }}
        >
          <h2 className={`text-2xl font-black mb-1 fade-up-2 ${t.text}`}>Join the fun! 🎉</h2>
          <p className={`text-sm mb-7 fade-up-2 ${t.muted}`}>Create your QuizPop account</p>
          ...
          <div className="fade-up-3">
            ...
            <div className="mb-5">
              ...
              <div className="relative">
                <span className={`absolute left-4 top-1/2 -translate-y-1/2 text-base ${t.muted} transition-colors duration-200`}>
                  🔑
                </span>
                <input
                  type="password"
                  ...
                  className={`w-full pl-11 pr-10 py-3.5 rounded-2xl border-2 outline-none transition-all duration-200 text-sm ${t.inp} ${
                    passwordMismatch ? "border-red-500 ring-4 ring-red-500/10" : ""
                  } ${passwordMatch ? "border-green-500 ring-4 ring-green-500/10" : ""}`}
                />
                {passwordMatch && (
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-green-400 text-base">✓</span>
                )}
                {passwordMismatch && (
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-red-400 text-base">✗</span>
                )}
              </div>
              {passwordMismatch && (
          ...
          <button
            ...
            disabled={loading || !email || !password || !confirm || passwordMismatch}
        ...
        {/* Footer */}
        <p className={`text-center mt-6 text-sm fade-up-5 ${t.muted}`}>
